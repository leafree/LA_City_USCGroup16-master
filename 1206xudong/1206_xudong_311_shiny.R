...
server <- function(input, output) {
    ...
    output$map1_bar <- renderPlot({
        ...
        labs(x = "Month", y = "Service Counts", 
             title = paste("Response Level  (",input$res_level, ")  Counts by Quarter & SPA", collapse = ""))+
...
#        geom_label(stat = "count", aes(label = ..count.., y = ..count..))+
    ...
    output$map2_bar <- renderPlot({
        ggplot(data = data_311call_selected2_res_level(), #[data_311call_selected()$SPA %in% c(4,5,6),], 
